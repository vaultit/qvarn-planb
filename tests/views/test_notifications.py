...
def test_listeners_id_put(client):
    ...
    result = client.put(f'/orgs/listeners/{LISTENER_2}', json={'notify_of_new': False, 'listen_on': [ORG_1], 'revision': REVISION_1})
    assert result.status_code == 200
    ...
    assert result['notify_of_new'] == False
    ...
    assert notification['type'] == 'notification'
    assert notification['resource_id'] == ORG_1
    assert notification['resource_revision'] == None
